...
ALLOWED_HOOK_FILES = {
    ...
    # `hosts/codex.py`, `hosts/cursor.py` and `hosts/copilot.py` are other clients'
    # records: inert here, since the shim always passes `--host opencode` and
    # `run.py` imports only the record it is given. Present because the tree is
    # copied whole with zero transforms, and named rather than wildcarded so a
    # file nobody read cannot ship from this plugin.
    "hosts/__init__.py", "hosts/claude.py", "hosts/codex.py", "hosts/opencode.py",
    "hosts/cursor.py", "hosts/copilot.py",
    ...
    # runs only when the first extractor is `claude`; with `--host opencode` this client's
